...
COURT_LINES = [
    ("baseline_back",   "Linea de fondo cercana",      "x", 0.0,                     "calibration"),
    ("baseline_front",  "Linea de fondo lejana",       "x", COURT_LENGTH_M,           "calibration"),
    ("midcourt",        "Medio campo",                 "x", COURT_LENGTH_M / 2,       "calibration"),
    ("ft_near",         "Tiro libre fondo cercano",    "x", 5.8,                      "calibration"),
    ("ft_far",          "Tiro libre fondo lejano",     "x", COURT_LENGTH_M - 5.8,     "calibration"),
    ("midcourt_vp",     "Medio campo solo fuga",       "x", COURT_LENGTH_M / 2,       "vanishing"),
    ("baseline_back_vp","Fondo cercana solo fuga",     "x", 0.0,                      "vanishing"),
    ("baseline_front_vp","Fondo lejana solo fuga",     "x", COURT_LENGTH_M,           "vanishing"),
    ("sideline_left",   "Lateral izquierdo",           "y", 0.0,                      "calibration"),
    ("sideline_right",  "Lateral derecho",             "y", COURT_WIDTH_M,            "calibration"),
    ("sideline_left_vp","Lateral izquierdo solo fuga", "y", 0.0,                      "vanishing"),
    ("sideline_right_vp","Lateral derecho solo fuga",  "y", COURT_WIDTH_M,            "vanishing"),
]
...
def _build_court_3d_lines() -> list[tuple[tuple[float, float, float], tuple[float, float, float]]]:
    ...
    for bx, direction in ((1.2, 1.0), (L - 1.2, -1.0)):
        cy = W / 2.0
        BW, BH, BZ = 1.83, 1.05, 3.05
        ...
        # Rim: horizontal circle at basket height in XY plane
        RX = bx + direction * 0.15
        RR = 0.225
        RN = 20
        for i in range(RN):
            a1 = 2 * math.pi * i / RN
        ...
        # Free throw lane (4.9 m wide, 5.8 m deep)
        LW = 4.9
        FTX = bx + direction * 5.8
...
COURT_3D_LINES = _build_court_3d_lines()
...
class CalibratorApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.camera_height_m = tk.DoubleVar(value=CAMERA_HEIGHT_M)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(top, text="Superponer 3D", variable=self.show_3d_overlay, command=self._render).grid(row=0, column=5, rowspan=2, padx=(12, 4))
        ttk.Label(top, text="Alt. cam (m)").grid(row=0, column=6, sticky="s", padx=(4, 2))
        height_spin = ttk.Spinbox(top, from_=0.5, to=20.0, increment=0.1, textvariable=self.camera_height_m, width=6)
        height_spin.grid(row=1, column=6, sticky="n", padx=(4, 2))
        self.camera_height_m.trace_add("write", lambda *_: self.after_idle(self._render))
    ...
    def _refresh_tool_options(self) -> None:
        ...
        if self.tool.get() == "court":
            self.court_line = tk.StringVar(value=COURT_LINES[0][0])
            line_combo = ttk.Combobox(self.options, state="readonly", values=[label for _, label, *_ in COURT_LINES])
            ...
            legend = ttk.Frame(self.options)
            legend.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(10, 0))
            for row, (code, label, _axis, _value, usage) in enumerate(COURT_LINES):
                tk.Label(legend, text="■", fg=COURT_LINE_COLORS[code], font=("Segoe UI", 10)).grid(row=row, column=0, sticky="w")
                suffix = " (solo fuga)" if usage == "vanishing" else ""
                ttk.Label(legend, text=f"{label}{suffix}").grid(row=row, column=1, sticky="w", padx=(4, 0))
    ...
    def _set_court_line(self, index: int) -> None:
        self.court_line.set(COURT_LINES[index][0])
    ...
    def _add_point(self, image_point: tuple[float, float]) -> None:
        ...
        if point_type == "court":
            ...
            code, label, axis, value, usage = COURT_LINES[line_index]
    ...
    def _draw_plane_grid(self) -> None:
        ...
        for x in np.arange(0.0, COURT_LENGTH_M + 0.001, 1.0):
            a = project(float(x), 0.0)
            b = project(float(x), COURT_WIDTH_M)
            if a and b:
                color = "#ffffff" if abs(x % 7.0) < 1e-6 else GRID_COLOR
                self.canvas.create_line(a[0], a[1], b[0], b[1], fill=color, width=1, dash=(2, 6))

        for y in np.arange(0.0, COURT_WIDTH_M + 0.001, 1.0):
            a = project(0.0, float(y))
            b = project(COURT_LENGTH_M, float(y))
            if a and b:
                color = "#ffffff" if y in (0.0, COURT_WIDTH_M) else GRID_COLOR
                self.canvas.create_line(a[0], a[1], b[0], b[1], fill=color, width=1, dash=(2, 6))
    ...
    def _draw_top_view(self) -> None:
        ...
        pad = 18
        scale = min((width - pad * 2) / COURT_LENGTH_M, (height - pad * 2) / COURT_WIDTH_M)
        ox = (width - COURT_LENGTH_M * scale) / 2
        oy = (height - COURT_WIDTH_M * scale) / 2
        self.top_view.create_rectangle(ox, oy, ox + COURT_LENGTH_M * scale, oy + COURT_WIDTH_M * scale, outline="#5f6b7a", width=2)
        self.top_view.create_line(ox + COURT_LENGTH_M / 2 * scale, oy, ox + COURT_LENGTH_M / 2 * scale, oy + COURT_WIDTH_M * scale, fill="#5f6b7a")
        for code, _label, axis, value, usage in COURT_LINES:
            if usage != "calibration":
                continue
            color = COURT_LINE_COLORS[code]
            if axis == "x":
                x = ox + value * scale
                self.top_view.create_line(x, oy, x, oy + COURT_WIDTH_M * scale, fill=color, width=2)
            else:
                y = oy + value * scale
                self.top_view.create_line(ox, y, ox + COURT_LENGTH_M * scale, y, fill=color, width=2)
    ...
    def _court_intersections(self, points: list[MarkPoint] | None = None) -> list[dict[str, Any]]:
        line_meta = {code: {"label": label, "axis": axis, "value": value, "usage": usage} for code, label, axis, value, usage in COURT_LINES}
    ...
    def _court_vanishing_points(self, points: list[MarkPoint] | None = None) -> dict[str, dict[str, float] | None]:
        line_meta = {code: {"axis": axis, "value": value, "usage": usage} for code, _label, axis, value, usage in COURT_LINES}
        groups = self._court_line_groups(points)
        image_lines = [
    ...
    def _image_line_from_points(self, points: list[MarkPoint]) -> tuple[float, float, float] | None:
        if len(points) < 2:
            return None
        p1, p2 = points[0], points[1]
        x1, y1 = p1.image["x"], p1.image["y"]
        x2, y2 = p2.image["x"], p2.image["y"]
        a = y1 - y2
        b = x2 - x1
        c = x1 * y2 - x2 * y1
        norm = math.hypot(a, b)
    ...
    def _provisional_three_line_correspondences(
        ...
        groups = self._court_line_groups(points)
        meta = {code: {"axis": axis, "value": value, "usage": usage} for code, _label, axis, value, usage in COURT_LINES}
    ...
    def _provisional_from_two_y_one_x(
        ...
        for y_code in y_lines:
            ...
            target_x = COURT_LENGTH_M if line_x <= COURT_LENGTH_M / 2 else 0.0
            scale = abs(target_x - line_x) / max(COURT_LENGTH_M, 1e-9)
    ...
    def _provisional_from_two_x_one_y(
        ...
        for x_code in x_lines:
            ...
            sample = q2 if self._distance(q2, anchor) > self._distance(q1, anchor) else q1
            direction = (sample[0] - anchor[0], sample[1] - anchor[1])
            target_y = COURT_WIDTH_M if line_y <= COURT_WIDTH_M / 2 else 0.0
            scale = abs(target_y - line_y) / max(COURT_WIDTH_M, 1e-9)
            synthetic = (anchor[0] + direction[0] / max(scale, 1e-6), anchor[1] + direction[1] / max(scale, 1e-6))
    ...
    def _fspy_like_intrinsics(
        ...
        principal = np.asarray([width / 2, height / 2], dtype=float)
        vp_x = vanishing_points.get("x")
        vp_y = vanishing_points.get("y")
        source = "fallback_fov"
        camera = self.active_camera.get()
        fov_fallback = CAMERA_FOV_HINTS.get(camera, DEFAULT_CAMERA_FOV_DEG)
        focal = (width / 2) / math.tan(math.radians(fov_fallback) / 2)
    ...
    def _estimate_camera_pnp(self, camera_name: str | None = None) -> dict[str, Any] | None:
        ...
        if len(backboard_pts) >= 4:
            ...
            basket_x = 1.2 if (target and target[0] < COURT_LENGTH_M / 2) else COURT_LENGTH_M - 1.2
    ...
    def _get_overlay_pose(self) -> dict[str, Any] | None:
        ...
        if pose.get("status", "").startswith("estimated_from_court"):
            try:
                override_z = float(self.camera_height_m.get())
    ...
    def _project_world_to_screen(
        ...
        if P_cam[2] >= -0.05:
            return None
        p_h = K @ P_cam
        return self._image_to_screen(float(p_h[0] / p_h[2]), float(p_h[1] / p_h[2]))
    ...
    def _draw_3d_court_overlay(self) -> None:
        ...
        for start, end in COURT_3D_LINES:
            s = self._project_world_to_screen(start, K, R, t)
            e = self._project_world_to_screen(end, K, R, t)
            if s is None or e is None:
                continue
            z_avg = (start[2] + end[2]) / 2.0
            is_rim = z_avg > 2.8 and abs(start[1] - COURT_WIDTH_M / 2) < 1.5 and abs(end[1] - COURT_WIDTH_M / 2) < 1.5
            if is_rim:
    ...
    def _export_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "court": {"length_m": COURT_LENGTH_M, "width_m": COURT_WIDTH_M},
            "camera_rig": {
...
class Calibration3DWindow(tk.Toplevel):
    ...
    def _project(self, point: tuple[float, float, float]) -> tuple[float, float]:
        width = max(self.canvas.winfo_width(), 1)
        height = max(self.canvas.winfo_height(), 1)
        x = point[0] - COURT_LENGTH_M / 2
        y = point[1] - COURT_WIDTH_M / 2
        z = point[2]
    ...
    def _draw_floor(self) -> None:
        if self.show_camera_mapping.get():
            ...
            self._draw_poly(
                [(0, 0, 0), (COURT_LENGTH_M, 0, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 0), (0, COURT_WIDTH_M, 0)],
                outline="#d7dee9",
                fill="#17212d",
                width=2,
            )
        for x in np.arange(0.0, COURT_LENGTH_M + 0.001, 1.0):
            ...
            self._draw_line((float(x), 0, 0.02), (float(x), COURT_WIDTH_M, 0.02), color, 1)
        for y in np.arange(0.0, COURT_WIDTH_M + 0.001, 1.0):
            color = "#5b6b7d" if y not in (0.0, COURT_WIDTH_M) else "#d7dee9"
            self._draw_line((0, float(y), 0.02), (COURT_LENGTH_M, float(y), 0.02), color, 1)

        for code, label, axis, value, usage in COURT_LINES:
            if usage != "calibration":
                continue
            color = COURT_LINE_COLORS[code]
            if axis == "x":
                self._draw_line((value, 0, 0.06), (value, COURT_WIDTH_M, 0.06), color, 4)
                self._draw_text((value, COURT_WIDTH_M + 0.8, 0.2), label, color)
            else:
                self._draw_line((0, value, 0.06), (COURT_LENGTH_M, value, 0.06), color, 4)
                self._draw_text((COURT_LENGTH_M + 1.2, value, 0.2), label, color)

        self._draw_text((COURT_LENGTH_M / 2, COURT_WIDTH_M / 2, 0.3), "CANCHA 28m x 15m", "#ffffff")

    def _draw_camera_mapped_floor(self) -> None:
        mode = self.mapping_camera.get()
        textures = [self._mapping_texture(mode)] if mode != "mix" else [self._mapping_texture("cam2"), self._mapping_texture("cam1")]
        textures = [texture for texture in textures if texture is not None]
        if not textures:
            self._draw_poly(
                [(0, 0, 0), (COURT_LENGTH_M, 0, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 0), (0, COURT_WIDTH_M, 0)],
                outline="#d7dee9",
        ...
        step = 0.5
        for x in np.arange(0.0, COURT_LENGTH_M, step):
            for y in np.arange(0.0, COURT_WIDTH_M, step):
                fill = self._sample_mapping_color(textures, x + step / 2, y + step / 2)
                self._draw_poly(
                    [(float(x), float(y), 0.01), (float(x + step), float(y), 0.01), (float(x + step), float(y + step), 0.01), (float(x), float(y + step), 0.01)],
                    outline=fill,
                    fill=fill,
                    width=1,
                )
        self._draw_poly(
            [(0, 0, 0.03), (COURT_LENGTH_M, 0, 0.03), (COURT_LENGTH_M, COURT_WIDTH_M, 0.03), (0, COURT_WIDTH_M, 0.03)],
            outline="#ffffff",
            fill="",
            width=2,
        )
    ...
    def _mapping_world_bounds(self, camera: dict[str, Any]) -> dict[str, float] | None:
        ...
        if len(intersections) >= 4:
            return {"min_x": 0.0, "max_x": COURT_LENGTH_M, "min_y": 0.0, "max_y": COURT_WIDTH_M}
        ...
        pad_x = 5.0 if len(x_values) <= 1 else COURT_LENGTH_M
        pad_y = 3.0 if len(y_values) <= 1 else COURT_WIDTH_M
        return {
            "min_x": max(0.0, min(xs) - pad_x),
            "max_x": min(COURT_LENGTH_M, max(xs) + pad_x),
            "min_y": max(0.0, min(ys) - pad_y),
            "max_y": min(COURT_WIDTH_M, max(ys) + pad_y),
        }

    def _draw_walls(self) -> None:
        wall_planes = self._wall_planes_present()
        if "left_wall" in wall_planes:
            self._draw_wall_frame([(0, COURT_WIDTH_M, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 5), (0, COURT_WIDTH_M, 5)], "Pared fondo")
        if "right_wall" in wall_planes:
            self._draw_wall_frame([(0, 0, 0), (COURT_LENGTH_M, 0, 0), (COURT_LENGTH_M, 0, 5), (0, 0, 5)], "Pared camaras")
        if "back_wall" in wall_planes:
            self._draw_wall_frame([(COURT_LENGTH_M, 0, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 0), (COURT_LENGTH_M, COURT_WIDTH_M, 5), (COURT_LENGTH_M, 0, 5)], "Pared fondo")
        if "front_wall" in wall_planes:
            self._draw_wall_frame([(0, 0, 0), (0, COURT_WIDTH_M, 0), (0, COURT_WIDTH_M, 5), (0, 0, 5)], "Pared fondo")
    ...
    def _draw_backboards(self) -> None:
        if not any(self._camera_points_of_type("backboard")):
            return
        for x, label in ((1.2, "Tablero A"), (COURT_LENGTH_M - 1.2, "Tablero B")):
            y = COURT_WIDTH_M / 2
            z = 3.05
            w = 1.8
            h = 1.05
    ...
    def _draw_cameras(self) -> None:
        ...
        fallback_targets = {
            "cam1": (1.2, COURT_WIDTH_M / 2 - 1.2, 3.05),
            "cam2": (COURT_LENGTH_M - 1.2, COURT_WIDTH_M / 2 + 1.2, 3.05),
        }
        for index, name in enumerate(names):
            ...
            target = CAMERA_TARGETS.get(name, fallback_targets.get(name, (1.2, COURT_WIDTH_M / 2, 3.05)))
    ...
    def _mapping_coverage(self, camera_name: str) -> float:
        ...
        homography = np.asarray(texture["homography"], dtype=float)
        height, width = image.shape[:2]
        for x in np.arange(0.5, COURT_LENGTH_M, 1.0):
            for y in np.arange(0.5, COURT_WIDTH_M, 1.0):
                total += 1
